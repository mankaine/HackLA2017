...
class CongressionalComprehension: 
    def __init__(self, gk, ppk):
        # INITIALIZE TITLE AND WINDOW 
        self._base_win = tk.Tk()
        self._base_win.title('Congressional Comprehension')
        self._base_win.configure(background=CONSTANT_BKG)

        # GIMME THE KEYS
        self._geocodio_key  = gk
        self._pro_publica_key = ppk
        ...
        self._street_address_text = tk.Label(master = self._base_win, font = UNIV_FONT, background=CONSTANT_BKG,
                                             textvariable = self._street_address_prompt)
        ...
        self._city_prompt_text = tk.Label(master = self._base_win, font = UNIV_FONT, background=CONSTANT_BKG,
                                          textvariable = self._city_prompt)
        ...
        self._state_prompt_text = tk.Label(master = self._base_win, font = UNIV_FONT, background=CONSTANT_BKG,
                                           textvariable = self._state_prompt)
        ...
        self._zip_code_prompt_text = tk.Label(master = self._base_win, font = UNIV_FONT, background=CONSTANT_BKG,
                                              textvariable = self._zip_code_prompt)
        ...
        # BUTTON
        self._confirm_address = tk.Button(master = self._base_win, text="Get Updates", relief=tk.GROOVE, background=CONSTANT_BKG)
    ...
    def _primary_button_clicked(self, e: tk.Event):
        ...
        if (self._address_valid()): 
            ...
            self.make_requests(self._geocodio_key, self._pro_publica_key)    
    ...
    def make_requests(self, geocodio_key, pro_publica_key):  
        _state_abbrv = (self._state_response.get().rstrip())
        self._clear_window()
        
        self._district = data_parse.DistrictImport(self._address, geocodio_key).get_district_number()
         
        print("Initializing politician objects")

        self._r0 = representative.Representative(str(self._district), _state_abbrv, pro_publica_key)
        self._s0 = senator.Senator(_state_abbrv, 0, pro_publica_key)
        self._s1 = senator.Senator(_state_abbrv, 1, pro_publica_key)
    ...
    def _display_politicians(self):
        ...
        rep_name.set(self._r0.get_name() + "\nRepresentative, District " + \
                    str(self._district) + "-" + self._address[-9:-7].upper())
        rep = tk.Label(master=self._base_win, font=UNIV_FONT, textvariable=rep_name, background="#819FF7")
        ...
        sen0 = tk.Label(master=self._base_win, font=UNIV_FONT, textvariable=sen0_name, background="#819FF7")
        ...
        sen1 = tk.Label(master=self._base_win, font=UNIV_FONT, textvariable=sen1_name, background="#819FF7")
...
#         print(ks)
#         pro_publica_public_key  = ks[0].strip("\n")
#         geocodio_public_key     = ks[1].strip("\n")
#         smmry_key               = ks[2].strip("\n")
#         cc = CongressionalComprehension(geocodio_public_key, pro_publica_public_key)
#         cc.run()
    pro_publica_public_key  = '4jVRBAKrhn4nniRoSo5Gf4AWuM8DaA9G3GUC9pqN'
    geocodio_public_key     = 'aee4500582f41015b15885e534ea55f28af5f82'
    smmry_key               = 'F03936BE6D'
    cc = CongressionalComprehension(geocodio_public_key, pro_publica_public_key)
    cc.run()
